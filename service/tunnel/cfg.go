...
func parseUsers(userList []config.TunnelUserConfig) (users []*url.Userinfo, err error) {
	if userList == nil {
		return
	}
	for _, user := range userList {
		...
		if user.Password == "" {
			users = append(users, url.User(user.Username))
		} else {
			users = append(users, url.UserPassword(user.Username, user.Password))
		}
	}
	return
}

func parseAuthenticator(users []config.TunnelUserConfig) (gost.Authenticator, error) {
	kvs := map[string]string{}
	for _, user := range users {
		kvs[user.Username] = user.Password
	}
	au := gost.NewLocalAuthenticator(kvs)
	return au, nil
}

func parseLimiter(users []config.TunnelUserConfig) (gost.Limiter, error) {
	if users == nil || len(users) <= 0 {
		return nil, nil
	}
	l, _ := gost.NewLocalLimiter("", "")
	for _, user := range users {
		err := l.AddRule(user.Username, user.Limiter)
		if err != nil {
			logger.Error("parse tunnel user limiter fail", map[string]interface{}{"user": user})
		}
	}
	return l, nil
}
