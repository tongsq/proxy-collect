...
type route struct {
	ServeNodes []config.TunnelConfig
	ChainNodes []dto.ProxyDto
	ChinLevel  int
	Retries    int
	Mark       int
}
...
func parseChainNode(nsList []dto.ProxyDto) (nodes []gost.Node, err error) {
	for _, nsNode := range nsList {
		ns := common.GetProxyUrl(&nsNode)
		node, err := gost.ParseNode(ns)
		if err != nil {
			logger.Error("parse chain node fail", map[string]interface{}{"ns": ns})
			continue
		}
		...
		if node.User == nil && nsNode.User != "" {
			if nsNode.Password == "" {
				node.User = url.User(nsNode.User)
			} else {
				node.User = url.UserPassword(nsNode.User, nsNode.Password)
			}
		}
		...
		if err != nil {
			logger.Error("loadCa fail", map[string]interface{}{"err": err})
			continue
		}
		tlsCfg := &tls.Config{
		...
		case "kcp":
			kpcConfig, err := parseKCPConfig(node.Get("c"))
			if err != nil {
				return nil, err
			}
			if kpcConfig == nil {
			...
			tr = gost.KCPTransporter(kpcConfig)
		case "ssh":
		...
		case "quic":
			...
			tr = gost.QUICTransporter(quicConfig)
		case "http2":
...
func (r *route) GenRouters() ([]router, error) {
	...
	for _, ns := range r.ServeNodes {
		nsUrl := common.GetTunnelUrl(&ns)
		node, err := gost.ParseNode(nsUrl)
		if err != nil {
			return nil, err
		}

		authenticator, err := parseAuthenticator(ns.Users)
		if err != nil {
			return nil, err
		}
		...
		if node.User == nil {
			if users, _ := parseUsers(ns.Users); len(users) > 0 {
				node.User = users[0]
			}
		}

		//init rate limiter
		limiterHandler, err := parseLimiter(ns.Users)
		if err != nil {
			return nil, err
		}
		...
		case "kcp":
			kcpConfig, er := parseKCPConfig(node.Get("c"))
			if er != nil {
				return nil, er
			}
			if kcpConfig == nil {
			...
			ln, err = gost.KCPListener(node.Addr, kcpConfig)
		case "ssh":
			sshConfig := &gost.SSHConfig{
			...
			if s := node.Get("ssh_authorized_keys"); s != "" {
				...
				sshConfig.AuthorizedKeys = keys
			}
			if node.Protocol == "forward" {
				ln, err = gost.TCPListener(node.Addr)
			} else {
				ln, err = gost.SSHTunnelListener(node.Addr, sshConfig)
			}
		case "quic":
			quicConfig := &gost.QUICConfig{
				TLSConfig:   tlsCfg,
			...
			ln, err = gost.QUICListener(node.Addr, quicConfig)
		case "http2":
...
func (r *router) Serve() error {
	logger.FInfo("%s on %s", r.node.String(), r.server.Addr())
